package br.com.springbatch.migracaodados.job;
...
public class MigracaoDadosJobConfig {
	...
	/* Realiza a chamada dos steps em sequencia
	@Bean
	...
	*/
	...
	/* Realiza a chamada dos steps em paralelo */
	@Bean
	public Job migracaoDadosJob(
			@Qualifier("migrarPessoaStep") Step migrarPessoaStep, 
			@Qualifier("migrarDadosBancariosStep") Step migrarDadosBancariosStep) {
		return jobBuilderFactory
				.get("migracaoDadosJob")
				.start(stepParalelo(migrarPessoaStep, migrarDadosBancariosStep))
				.end()
				.incrementer(new RunIdIncrementer())
				.build();
	}

	private Flow stepParalelo(Step migrarPessoaStep, Step migrarDadosBancariosStep) {
		Flow migrarDadosBancariosFlow = new FlowBuilder<Flow>("migrarDadosBancariosFlow")
				.start(migrarDadosBancariosStep)
				.build();
		
		Flow stepParalelo = new FlowBuilder<Flow>("stepParalelosFlow")
				.start(migrarPessoaStep)
